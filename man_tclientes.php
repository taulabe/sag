...
<head>
  <meta charset="UTF-8">
  <title>Gestionar Tipos de Clientes y Agregar Tipo de Cliente</title>
  ...
  <article style="margin-top:10px; margin-left:15px;">
    <div class="row">
      <div class="col-md-7">
        ...
        <div class="recuadro">
          <form action="add_tcliente.php" id="add_tcliente" method="post" data-toggle="validator">
            <div class="input-group input-group-sm" style="margin-bottom:5px;">
              <span class="input-group-addon" id="basic-addon1">
                <span class="glyphicon glyphicon-tag"></span> 
              </span>
              <input type="text" name="tcliente_desc" id="tcliente_desc" class="form-control" placeholder="Escriba un tipo de cliente" aria-describedby="basic-addon1" required>
              <input type="hidden" name="pro" value="<?php echo md5('tcliente'); ?>" >
            </div>
            <div class="botones_i">
              <button type="submit" id="agregar_tcliente" class="btn btn-default btn-sm">
                <span class="glyphicon glyphicon-floppy-save" style="margin-right:7px;"></span>Agregar Tipo de Cliente
              </button>
            </div>
          </form>
        </div>
  ...
  <article class="col-lg-12" style="margin-top:10px;">
    <table cellpadding="0" cellspacing="1" border="0" align="center" width="100%" id="table" class="sortable">
      ...
      <?php
        ...
        $hq = "SELECT count(*) as total FROM actividades.man_tcliente";
        $rhq = mysqli_query($link, $hq);
        $hqr = mysqli_fetch_array($rhq);
        $trhq = $hqr['total'];

        if ($trhq > 0) {
          $qry1 = mysqli_query($link, "SELECT * FROM actividades.man_tcliente"); 
