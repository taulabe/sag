...
<head>
  <meta charset="UTF-8">
  <title>Gestionar Correlativos KBD y Agregar Correlativo</title>
...
<section class="main row" style="font-size:12px;">
  <!-- AGREGAR CORRELATIVO KBD -->
  <article style="margin-top:10px; margin-left:15px;">
    <div class="row">
      <div class="col-md-7">
        <strong style="font-size:14px;">Informaci&oacute;n General</strong>
        <div class="recuadro">
          <form action="add_correlativokbd.php" id="add_correlativokbd" method="post" data-toggle="validator">
            <div class="input-group input-group-sm" style="margin-bottom:5px;">
              <span class="input-group-addon" id="basic-addon1">
                <span class="glyphicon glyphicon-tag"></span> 
              </span>
              <input type="text" name="correlativokbd" id="correlativokbd" class="form-control" placeholder="Escriba un correlativo" aria-describedby="basic-addon1" required>
              <input type="hidden" name="pro" value="<?php echo md5('correlativokbd'); ?>" >
            </div>
            <div class="botones_i">
              <button type="submit" id="agregar_correlativokbd" class="btn btn-default btn-sm">
                <span class="glyphicon glyphicon-floppy-save" style="margin-right:7px;"></span>Agregar Correlativo
              </button>
            </div>
          </form>
        </div>
  ...
  <article class="col-lg-12" style="margin-top:10px;">
    <table cellpadding="0" cellspacing="1" border="0" align="center" width="130%" id="table" class="sortable">
      ...
      <?php
        ...
        $hq = "SELECT count(*) as total FROM actividades.ctl_correlativokbd";
        $rhq = mysqli_query($link, $hq);
        $hqr = mysqli_fetch_array($rhq);
        $trhq = $hqr['total'];

        if ($trhq > 0) {
          $qry1 = mysqli_query($link, "SELECT * FROM actividades.ctl_correlativokbd"); 
