...
<head>
  <meta charset="UTF-8">
  <title>Gestionar KB Documentos</title>
    ...
    <table cellpadding="0" cellspacing="1" border="0" align="center" width="100%" id="table" class="sortable">
      <thead>
        <tr class="encabezado">
          ...
          <td width="200" align="center" valign="middle">Ruta</td>
      ...
      <?php
        ...
        $hq = "SELECT count(*) as total FROM actividades.man_kbdocumentos";
        $rhq = mysqli_query($link, $hq);
        $hqr = mysqli_fetch_array($rhq);
        $trhq = $hqr['total'];

        if ($trhq > 0) {
          $qry1 = mysqli_query($link, "SELECT d.idkbdocumento, d.kbdtitulo, d.kbdf_creado, d.kbdf_modificado, d.kbd_ruta FROM actividades.man_kbdocumentos d, actividades.man_tsolucion t WHERE d.idkbdtsolucion = t.idtkbsolucion"); 
          while ($row1 = mysqli_fetch_array($qry1)) {
            $cont++;
      ?>
        <tr style="font-size:12px;" bgcolor="<?php echo $color[$cont % 2]; ?>">
          <td height="6" align="center"><?php echo $row1['idkbdocumento']; ?></td>
          <td><?php echo $row1['kbdtitulo']; ?></td>
          <td><?php echo $row1['kbdf_creado']; ?></td>
          <td><?php echo $row1['kbdf_modificado']; ?></td>
          <td><?php echo $row1['kbd_ruta']; ?></td>
          <td align="center"> 
            <form action="procesos.php" method="get">
              <button type="submit" class="btn btn-default btn-xs">
                <input type="hidden" value="<?php echo $row1['idkbdocumento']; ?>" name="i" />
