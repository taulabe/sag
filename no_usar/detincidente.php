      ...
      <table cellpadding="0" cellspacing="1" border="0" align="center" width="130%" id="table" class="sortable">
        <thead>
          <tr class="encabezado">
            ...
            <td width="80" align="center" valign="middle">Nombre Cliente</td>
		...
		$num=0;
		if($idrol >= 3 || $idrol <= 4){
			$hq = "SELECT count(*) as total FROM actividades.v_incidente where idestatus in (5,6,7)";
		} else{
			if($idrol >= 1 || $idrol <= 2){
				$hq = "SELECT count(*) as total FROM actividades.v_incidente where (idcliente='$id' or idtecnico='$id') and idestatus in (5,6,7)";
			}
		}
		...
		if ( $trhq > 0 ){
			if($idrol == 3 || $idrol == 4){
				$qry1=mysqli_query($link,"SELECT * FROM actividades.v_incidente where idestatus in (5,6,7)"); 
			}
			if($idrol == 2){
				$qry1=mysqli_query($link,"select * from actividades.v_incidente where idcliente='$id' and idestatus in (5,6,7)"); 
			}
			if($idrol == 1){
				$qry1=mysqli_query($link,"select * from actividades.v_incidente where (idcliente='$id' or idtecnico='$id') and idestatus in (5,6,7);"); 
			}
		...
		?>
          <tr bgcolor="<?php echo $color[$cont%2]; ?>">
            ...
            <td align="center"><?php echo $row1['tecnicoasig'];?></td>
            <td align="center"><?php echo $row1['estdesc']; ?></td>
