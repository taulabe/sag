...
<head>
  <meta charset="UTF-8">
  <title>Gestionar Roles y Agregar Rol</title>
  ...
  <article style="margin-top:10px; margin-left:15px;">
    <div class="row">
      <div class="col-md-7">
        ...
        <div class="recuadro">
          <form action="add_rol.php" id="add_rol" method="post" data-toggle="validator">
            <div class="input-group input-group-sm" style="margin-bottom:5px;">
              <span class="input-group-addon" id="basic-addon1">
                <span class="glyphicon glyphicon-tag"></span> 
              </span>
              <input type="text" name="rol" id="rol" class="form-control" placeholder="Escriba un rol" aria-describedby="basic-addon1" required>
              <input type="hidden" name="pro" value="<?php echo md5('rol'); ?>" >
            </div>
            <div class="botones_i">
              <button type="submit" id="agregar_rol" class="btn btn-default btn-sm">
                <span class="glyphicon glyphicon-floppy-save" style="margin-right:7px;"></span>Agregar Rol
              </button>
            </div>
          </form>
        </div>
    ...
    <table cellpadding="0" cellspacing="1" border="0" align="center" width="130%" id="table" class="sortable">
      <thead>
        <tr class="encabezado">
          ...
          <td width="100" align="center" valign="middle">Rol</td>
      ...
      <?php
        ...
        $hq = "SELECT count(*) as total FROM actividades.man_roles";
        $rhq = mysqli_query($link, $hq);
        $hqr = mysqli_fetch_array($rhq);
        $trhq = $hqr['total'];

        if ($trhq > 0) {
          $qry1 = mysqli_query($link, "SELECT * FROM actividades.man_roles"); 
          while ($row1 = mysqli_fetch_array($qry1)) {
            $cont++;
      ?>
        <tr style="font-size:12px;" bgcolor="<?php echo $color[$cont % 2]; ?>">
          <td height="10" align="center"><?php echo $cont; ?></td>
          <td><?php echo $row1['rolnom']; ?></td>
          <td align="center"> 
            <form action="procesos.php" method="get">
              <button type="submit" class="btn btn-default btn-xs">
                <input type="hidden" value="<?php echo $row1['idrol']; ?>" name="i" />
                <input type="hidden" value="<?php echo md5(4); ?>" name="a" />
                <span class="glyphicon glyphicon-wrench" title="Ver rol" aria-hidden="true"></span>
              </button>
            </form>
          </td>
        </tr>
