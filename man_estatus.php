...
<head>
  <meta charset="UTF-8">
  <title>Gestionar Estatus y Agregar Estatus</title>
    ...
    <table cellpadding="0" cellspacing="1" border="0" align="center" width="130%" id="table" class="sortable">
      <thead>
        <tr class="encabezado">
          ...
          <td width="100" align="center" valign="middle">Estatus</td>
      ...
      <?php
        ...
        $hq = "SELECT count(*) as total FROM actividades.man_estatus";
        $rhq = mysqli_query($link, $hq);
        $hqr = mysqli_fetch_array($rhq);
        $trhq = $hqr['total'];

        if ($trhq > 0) {
          $qry1 = mysqli_query($link, "SELECT * FROM actividades.man_estatus"); 
          while ($row1 = mysqli_fetch_array($qry1)) {
            $cont++;
      ?>
        <tr style="font-size:12px;" bgcolor="<?php echo $color[$cont % 2]; ?>">
          <td height="10" align="center"><?php echo $cont; ?></td>
          <td><?php echo $row1['estdesc']; ?></td>
          <td align="center"> 
            <form action="procesos.php" method="get">
              <button type="submit" class="btn btn-default btn-xs">
                <input type="hidden" value="<?php echo $row1['idestatus']; ?>" name="i" />
                <input type="hidden" value="<?php echo md5(4); ?>" name="a" />
                <span class="glyphicon glyphicon-wrench" title="Ver estatus" aria-hidden="true"></span>
              </button>
            </form>
          </td>
        </tr>
